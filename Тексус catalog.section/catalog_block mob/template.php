...
<? if ($arResult["ITEMS"]) : ?>
	...
	if ($bSlider) {
		...
		$mobileSpecialClass = "mobile-class-for-prices"; // добавил переменную чтобы выводилась в мобильной версии в листе товаров только в карточке товара
	} else {
				...
				<? if ($bSlider) : ?>
					<div class="js_append ajax_load appear-block <?= $sliderClasses ?>" data-p="<?= $arParams['SHOW_GALLERY'] ?>">
						...
						<? foreach ($arResult["ITEMS"] as $arItem) {
							...
							<div class="catalog-block__wrapper<?= $elementSliderClasses; ?> <?= $mobileSpecialClass ?> grid-list__item grid-list-border-outer <?= ($arCurrentOffer ? 'has-offers' : ''); ?>" data-hovered="false">
								...
								<div class="catalog-block__item <?= $itemClass ?>" id="<?= $arItem["strMainID"] ?>">
									...
									<div class="catalog-block__inner flexbox height-100" <? if ($bUseSchema) : ?>itemprop="itemListElement" itemscope="" itemtype="http://schema.org/Product" <? endif; ?>>
										...
										<? endif; ?>
										<div class="catalog-block__info flex-1 flexbox flexbox--justify-beetwen" data-id="<?= ($arCurrentOffer ? $arCurrentOffer['ID'] : $arItem['ID']) ?>" data-item="<?= $dataItem; ?>" <? if ($bUseSchema) : ?>itemprop="offers" itemscope itemtype="http://schema.org/Offer" <? endif; ?>>
											<div class="catalog-block__info-top">
												<div class="catalog-block__info-inner">
													...
													<div class="js-popup-price" data-price-config='<?= str_replace('\'', '"', CUtil::PhpToJSObject($arPriceConfig, false, true)) ?>'>
														<!-- цены образца заменил + условие для товаров со скидкой в листе товаров -->
														<? if (!str_contains($arPrices['ECONOMY'], '%')) : ?>
															<? if ($mobileSpecialClass) :  // а тут эту переменную с классами для обертки элемента
															?>
																<span class="price__new-val font_18 font_14--to-600"><b><? if ($arItem["OFFERS"][1]["MIN_PRICE"]["PRINT_VALUE"]) : ?>от <?= $arItem["OFFERS"][1]["MIN_PRICE"]["PRINT_VALUE"] ?><? else : ?><?= $priceHtml ?><? endif ?></b></span>
															<? else : ?>
																<span class="price__new-val font_18 font_14--to-600"><b><?= $priceHtml ?></b></span>
															<? endif; ?>
														<? else : ?>
															<span class="price__new-val font_18 font_14--to-600"><b><?= $priceHtml ?></b></span>
														<? endif; ?>
											...
											<div class="catalog-block__info-bottom <?= ($arCurrentOffer ? 'catalog-block__info-bottom--with-sku' : ''); ?>">
												...
												<? if ($arItem['SKU']['PROPS'] && !$bMobileScrolledItems) : ?>
													<div class="catalog-block__offers hide-600">
														<div class="sku-props sku-props--block" data-site-id="<?= SITE_ID; ?>" data-item-id="<?= $arItem['ID']; ?>" data-iblockid="<?= $arItem['IBLOCK_ID']; ?>" data-offer-id="<?= $arCurrentOffer['ID']; ?>" data-offer-iblockid="<?= $arCurrentOffer['IBLOCK_ID']; ?>" data-offers-id='<?= str_replace('\'', '"', CUtil::PhpToJSObject($GLOBALS[$arParams['FILTER_NAME']]['OFFERS_ID'], false, true)) ?>'>
